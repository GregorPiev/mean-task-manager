  ...
  if ((queryString.username == undefined) || (queryString.password == undefined)) {
    ...
    var dataQuery = databaseConnection.executeQuery(query, function (results) {
      ...
      if (results.length == 1) {
        ...
        resultObject.result = {};
        resultObject.result.userId = results[0].RowDataPacket.userId;
        resultObject.result.username = results[0].RowDataPacket.userName;
        resultObject.result.roleId = results[0].RowDataPacket.roleId;
      } else {
        resultObject.error = 1;
      }
