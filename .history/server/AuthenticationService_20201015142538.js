...
function login(response, queryString) {
  ...
  if ((queryString.username == undefined) || (queryString.password == undefined)) {
    ...
    var dataQuery = databaseConnection.executeQuery(query, function (results) {
      ...
      if (results.length == 1) {
        const { userId, userName, password, roleId } = results[0];
        console.log('Login executeQuery assignment:' + userId + '  ' + userName + '  ' + password + ' ' + roleId)
        resultObject.error = 0;
        resultObject.result = {};
        resultObject.result.userId = new String(userId);
        resultObject.result.username = userName;
        resultObject.result.password = password;
        resultObject.result.roleId = new String(roleId);
      } else {
        resultObject.error = 1;
      }
